 ...
 */
export const wifiPortal = functions.https.onRequest(async (req, res) => {
  corsHandler(req, res, async () => {
    try {
      ...
      const branding = await getGymBranding(gym as string);
 ...
 */
export const wifiAuth = functions.https.onRequest(async (req, res) => {
  corsHandler(req, res, async () => {
    try {
 ...
 */
export const wifiStatus = functions.https.onRequest(async (req, res) => {
  corsHandler(req, res, async () => {
    try {
...
async function getGymBranding(gymId?: string): Promise<GymBranding> {
  const defaultBranding: GymBranding = {
...
async function getMemberInfo(uid: string, gymCode?: string): Promise<MemberInfo | null> {
  try {
...
async function generateWiFiWiFiToken(uid: string, gymId: string): Promise<string> {
...
async function verifyWiFiToken(token: string): Promise<any> {
  try {
...
async function getWiFiUsageStats(sessionId: string): Promise<{ download: number; upload: number }> {
  // Simular estadísticas de uso
  return {
    download: Math.floor(Math.random() * 500), // MB
    upload: Math.floor(Math.random() * 100)    // MB
  };
}

function getSessionDuration(membershipType: string): number {
